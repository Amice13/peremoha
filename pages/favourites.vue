<template>
  <v-row justify="center" align="center">
    ...
    <div cols="12" class="ma-0 mt-8" style="width: 100%">
      <div class="tt fw-400 text--primary fs-175 uppercase mb-4 text-center w-100">Обране</div>
        <v-col cols="12" sm="12" class="py-2 text-center">
          <v-btn-toggle v-model="filters" shaped multiple>
            <v-btn
              v-for="(item, key) in filtersList"
              :key="item.id"
              :color="filters.includes(key) ? 'yellow' : 'primary'"
        ...
        <v-card width="100%" class="elevation-0 text-center mb-8" v-show="!favourites.length">
          <v-card-text>
            <div>Ви ще не додали в обране жодний з лотів. Прямо зараз скористайтеся пошуком, щоб знайти те, що вам сподобається!</div>
          </v-card-text>
          <v-card-actions>
            <v-spacer v-show="$vuetify.breakpoint.mdAndUp"></v-spacer>
            <v-btn router to="/search" color="primary" depressed :block="$vuetify.breakpoint.smAndDown">
              Пошук лотів
            </v-btn>
            <v-spacer v-show="$vuetify.breakpoint.mdAndUp"></v-spacer>
          </v-card-actions>
    ...
    <v-card width="100%" class="elevation-0" v-show="favourites.length">
      <v-card-text>
...
export default {
  ...
  computed: {
    ...
    filteredItems: function () {
      ...
      if (this.favourites.length) items = items.filter(el => this.favourites.includes(el.id))
      return items
    },
